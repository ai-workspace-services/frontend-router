...
describe('brand domains', () => {
  ...
  for (const host of ['xworktech.com', 'www.xworktech.com']) {
    ...
    // domain, including the anonymous trial entry.
    ...
    for (const path of ['/ai-workspace?entry=trial', '/xworkmate', '/cloud_iac', '/editor']) {
  ...
  it('keeps public workspace APIs on the brand domain', async () => {
    for (const path of ['/api/ai-workspace/trial', '/api/xworkmate/bridge']) {
      const runtime = env(website);
      const response = await worker.fetch(new Request(`https://www.xworktech.com${path}`, { method: 'POST', body: '{}' }), runtime);
      expect(response.status).toBe(200);
      expect(response.headers.get('Location')).toBeNull();
